...
def adjust_version(build_number, git_hash):
    ...
    version = None
    with open("lib/c-api/include/clingcon.h", encoding="utf-8") as fh:
        for line in fh:
            m = re.match(r'#define CLINGCON_VERSION "([0-9]+\.[0-9]+\.[0-9]+)"', line)
            if m is not None:
                version = m.group(1)
    ...
    pyproject["project"]["urls"]["Source"] = (
        f"https://github.com/potassco/clingcon/tree/{git_hash}"
    )
