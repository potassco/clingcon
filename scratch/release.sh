...
"${CMAKE}" \\
    ...
    -G "${GENERATOR}" \\
    -DCMAKE_BUILD_TYPE=Release \\
    -DCLINGCON_BUILD_SHARED=OFF \\
    -DCLINGO_BUILD_WITH_PYTHON=OFF \\
    -DCLINGO_BUILD_APPS=OFF \
    -DCLINGO_REQUIRE_LUA=ON \\
    -DCLINGCON_BUILD_STATIC=ON \\
...
The \`clingcon.exe\` binary is compiled with Lua 5.3 but
without Python support.

- \`clingcon.exe\`: solver for non-ground programs
EOF
...
if [[ ! -e ${SRC} ]]; then
    git clone --branch "$branch" --single-branch --depth=1 git@github.com:potassco/clingcon ${SRC}
...
(setopt NULL_GLOB; rm -rf ${SRC}/{.ycm_extra_conf.py*,.travis.yml,.git*,scratch,TODO,Makefile,clingo,appveyor.yml,format.sh})
zip -r "${SRC}.zip" "${SRC}"
