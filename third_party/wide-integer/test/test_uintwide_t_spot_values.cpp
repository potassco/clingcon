...
namespace local {
template <typename UnknownIntegerType> bool test_uintwide_t_spot_values_from_pull_request_130() {
...
bool math::wide_integer::test_uintwide_t_spot_values() {
...

    {
        ...
        static_assert(WIDE_INTEGER_CONSTEXPR_IS_COMPILE_TIME_CONST == 1,
                      "Error: clang constexpr is not properly configured");
#endif
#endif
    }
...

    {
        // See also https://github.com/ckormanyos/wide-integer/issues/108
        {
            ...
            }
            {
                std::stringstream strm;
                strm << neg_wide;
                str_neg_wide = strm.str();
            }
            {
                std::stringstream strm;
                strm << neg_wide_cast;
                str_neg_wide_cast = strm.str();
            }
            ...
            result_is_ok &= (result_neg_is_ok && result_neg_wide_is_ok && result_neg_wide_cast_is_ok);
        }
...

    {
        ...
        WIDE_INTEGER_CONSTEXPR auto input{
            math::wide_integer::uintwide_t<320U, std::uint32_t, void, true>{1729348762983LL}};
...

    {
        ...
        const uint512_t a("69893733979034754305379740056436611874431253713844560791954862817582211580581298395579432130"
                          "4304417541511379093392776018867245622409026835324102460829431");
    ...
    {
        using uint256_t = math::wide_integer::uintwide_t<256U, std::uint32_t>;

        static_assert(std::numeric_limits<uint256_t>::digits == 256, "Error: Incorrect digit count for this example");
