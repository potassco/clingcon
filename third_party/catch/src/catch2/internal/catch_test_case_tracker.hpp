...
namespace Catch {
    namespace TestCaseTracking {
        ...
        struct NameAndLocation {
            ...
            NameAndLocation( std::string&& _name,
                             SourceLineInfo const& _location );
            friend bool operator==( NameAndLocation const& lhs,
                                    NameAndLocation const& rhs ) {
                // This is a very cheap check that should have a very high hit
                // rate. If we get to SourceLineInfo::operator==, we will redo
                // it, but the cost of repeating is trivial at that point (we
                // will be paying multiple strcmp/memcmps at that point).
        ...
        /**
         * This is a variant of `NameAndLocation` that does not own the name
         * string
         *
        ...
        class SectionTracker : public TrackerBase {
            ...
            // `ITracker` parent`. Currently it allocates owns the name, so this
            // is safe. If it is later refactored to not own the name, the name
            // still has to outlive the `ITracker` parent, so this should still
            // be safe.
