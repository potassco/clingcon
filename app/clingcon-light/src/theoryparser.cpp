...
namespace clingcon
{
    ...
    std::stringstream& TheoryParser::toString(std::stringstream& ss, const Clingo::TheoryTerm& t)
    {
        if (t.type()==Clingo::TheoryTermType::Number)
            ...
            if (t.type()==Clingo::TheoryTermType::Symbol)
                ...
                {
                    if (t.type()==Clingo::TheoryTermType::Function || t.type()==Clingo::TheoryTermType::Tuple)
                    {
                        ss << t.name();
                        ss << "(";
                    }

                    if (t.type()==Clingo::TheoryTermType::Set)
                        ss << "{";

                    for (auto i = t.arguments().begin(); i != t.arguments().end(); ++i)
                    {
                        toString(*i);
                        if (i != t.arguments().end()-1)
                            ss << ",";
                    }
    ...
    {
        return ((a.type()==Clingo::TheoryTermType::Function || a.type()==Clingo::TheoryTermType::Symbol) && check(a));
    }
            ...
            if (a.type()==Clingo::TheoryTermType::Function)
            {
                ...
                if (!isalpha(fname[0]))
                {
                    ...
                    if (fname=="+" && a.arguments().size()==2)
                    {
                        ...
                        {
                            v = createVar(a, getNumber(*a.arguments().begin()) + getNumber(*(a.arguments().begin()+1)));
                        }
                        else
                        {
                            if (isNumber(*a.arguments().begin()))
                            {
                                ...
                                if (isNumber(*(a.arguments().begin()+1)))
                                {
                                    bool b = getView(*(a.arguments().begin()),v);
    ...
    View TheoryParser::createVar(const Clingo::TheoryTerm& t)
    {
        ...
        termId2View_.resize(std::max((unsigned int)(termId2View_.size()), (unsigned int)(id+1)), View(InvalidVar));
    ...
    {
        auto x = find_if(string2view_.begin(),string2view_.end(),[&v](const std::pair<std::string,View>& vt)
...
//                std::cout << std::endl;
    ...
    case DOM:
    {
                ...
                if (op.name()==".." && op.arguments().size()==2)
                {
                    if (isNumber(*op.arguments().begin()))
                    {
                        if (isNumber(*(op.arguments().begin()+1)))
                            d.unify(getNumber(*op.arguments().begin()),getNumber(*(op.arguments().begin()+1)));
                        else
        ...
        View v;
        if (!getView(i->guard().second, v))
            error("Rhs VariableView expected",i->guard().second);
        ...
        for (auto elem = i->elements())
        {
            ...
            auto single_elem = elem.tuple().begin();
            {
                ...
                if (op.type() == Clingo::TheoryTermType::Function)
                {
                    if (op.name()=="/")
                    {
                        if (op.arguments().size()==2 && isVariable(*op.arguments().begin()) && (op.begin()+1)->type()==Clingo::TheoryTermType::Number)
                        {
                          CONTINUE HERE
                ...
                {
                    shown_.resize(std::max((unsigned int)(v.v)+1,(unsigned int)(shown_.size())),std::make_pair(InvalidVar,Clasp::Literal(0,false)));
        ...
        for (auto elemId = (*i)->begin(); elemId != (*i)->end(); ++elemId)
        {
            ...
            if (op.isFunction())
            {
                ...
                if (toString(term)=="@")
                {
                    if (op.size()==2)
                    {
                        if (getView(*op.begin(),v) && td_.getTerm(*(op.begin()+1)).type()==Clingo::TheoryTermType::Number)
                        {
...
void TheoryParser::add2Shown(Variable v, uint32 tid, Clasp::Literal l)
{
...
NameList& TheoryParser::postProcess()
{
    for (auto i : shownPred_)
    {
        ...
        Clingo::id_t function = *predTerm.begin();
        ...
        for (uint32 tid = 0; tid != termId2View_.size(); ++tid)
        {
            ...
            if (v.v!=InvalidVar)
            {
                ...
                if ( (term.isFunction() && term.size()==arity && term.function() == function) ||
                     (arity==0 && term.type()==Clingo::TheoryTermType::Symbol && tid == function) )
                {
