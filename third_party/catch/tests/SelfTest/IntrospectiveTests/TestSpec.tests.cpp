...
TEST_CASE( "Parse test names and tags",
           "[command-line][test-spec][approvals]" ) {
    ...
    SECTION( "Shortened hide tags also properly handle exclusion" ) {
        ...
        CHECK( spec.matches(
            *fakeTestCase( "neither foo nor hidden", "[bar]" ) ) );
    }
}
...
TEST_CASE( "Test spec serialization is round-trippable",
           "[test-spec][serialization][approvals]" ) {
    using Catch::parseTestSpec;
    using Catch::TestSpec;
    ...
    SECTION( "Multiple disjunct filters" ) {
        CHECK( serializedTestSpec( "[abc],[def]" ) == "[abc],[def]" );
        CHECK( serializedTestSpec( "[def],[abc],[idkfa]" ) ==
               "[def],[abc],[idkfa]" );
    }
