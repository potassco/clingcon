...
def adjust_version():
    ...
    for m in finditer(r'clingcon-{}.*manylinux2014_{}'.format(escape(version), escape(ARCH)), pip):
        post = max(post, 1)

    for m in finditer(r'clingcon-{}.post([0-9]+).*manylinux2014_{}'.format(escape(version), escape(ARCH)), pip):
