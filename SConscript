...
LIBOPTS_SOURCES = find_files(env, 'clingo/libprogram_opts/src')
LIBOPTS_HEADERS = [Dir('#clingo/libprogram_opts'), Dir('#clingo/libprogram_opts/src')]
...
LIBLP_SOURCES = find_files(env, 'clingo/liblp/src')
LIBLP_HEADERS = [Dir('#clingo/liblp'), Dir('#clingo/liblp/src')]
...
LIBCLASP_SOURCES = find_files(env, 'clingo/libclasp/src')
LIBCLASP_HEADERS = [Dir('#clingo/libclasp'), Dir('#clingo/libclasp/src')] + LIBOPTS_HEADERS + LIBLP_HEADERS
...
LIBGRINGO_SOURCES = find_files(env, 'clingo/libgringo/src')
LIBGRINGO_HEADERS = [Dir('#clingo/libgringo'), 'clingo/libgringo/src', 'clingo/libreify'] + LIBLP_HEADERS
...
LIBREIFY_SOURCES = find_files(env, 'clingo/libreify/src')
LIBREIFY_HEADERS = [Dir('#clingo/libreify'), 'clingo/libreify/src'] + LIBGRINGO_HEADERS
...
LIBCLINGO_SOURCES = find_files(env, 'clingo/libclingo/src')
LIBCLINGO_HEADERS = [Dir('#clingo/libclingo')] + LIBGRINGO_HEADERS + LIBCLASP_HEADERS
...
clingconProgramEnv.Prepend(LIBS=[ clingoLib, gringoLib, claspLib, optsLib, lpLib, reifyLib, clingconLib, orderLib ])
...
TEST_LIBGRINGO_SOURCES  = find_files(env, 'clingo/libgringo/tests')
...
TEST_LIBREIFY_SOURCES  = find_files(env, 'clingo/libreify/tests')
...
TEST_LIBLP_SOURCES  = find_files(env, 'clingo/liblp/tests')
...
TEST_CLINGO_SOURCES  = find_files(env, 'clingo/libclingo/tests')
...
clingoTestCommand = env.Command('clingo-test', clingconProgram, 'python clingo/app/clingo/tests/run.py -c $SOURCE run' + (" -- -t8" if env["WITH_THREADS"] is not None else ""))
