...
TEST_CASE( "SizedUnsignedType helpers", "[integer][approvals]" ) {
    ...
    STATIC_REQUIRE(
        std::is_unsigned<DoubleWidthUnsignedType_t<std::uint8_t>>::value );
    STATIC_REQUIRE( sizeof( DoubleWidthUnsignedType_t<std::uint16_t> ) == 4 );
    STATIC_REQUIRE(
        std::is_unsigned<DoubleWidthUnsignedType_t<std::uint16_t>>::value );
    STATIC_REQUIRE( sizeof( DoubleWidthUnsignedType_t<std::uint32_t> ) == 8 );
    STATIC_REQUIRE(
        std::is_unsigned<DoubleWidthUnsignedType_t<std::uint32_t>>::value );
}
...
TEST_CASE( "extendedMult 32x32", "[integer][approvals]" ) {
    ...
    CommutativeMultCheck<uint32_t>(
        0xdcdc'0000, 0x0000'aabb, 0x0000'934b, 0x6cb4'0000 );
...
TEST_CASE( "negative and positive signed integers keep their order after "
