...
namespace {
    auto fakeTestCase( const char* name, const char* desc = "" ) {
        return Catch::makeTestCaseInfo(
            "", { name, desc }, CATCH_INTERNAL_LINEINFO );
    }
} // namespace
    ...
    SECTION( "reporter" ) {
        ...
        SECTION( "must match one of the available ones" ) {
            ...
            REQUIRE_THAT( result.errorMessage(),
                          ContainsSubstring( "Unrecognized reporter" ) );
        }
        ...
        SECTION( "With Windows-like absolute path as output file" ) {
            auto result =
                cli.parse( { "test", "-r", "console::out=C:\\Temp\\out.txt" } );
            CAPTURE( result.errorMessage() );
            CHECK( result );
            REQUIRE( config.reporterSpecifications ==
        ...
        SECTION( "Multiple reporters" ) {
            SECTION( "All with output files" ) {
                CHECK( cli.parse( { "test",
                                    ...
                                    "xml::out=output.xml",
                                    "-r",
                                    "junit::out=output-junit.xml" } ) );
                REQUIRE(
                    config.reporterSpecifications ==
            ...
            SECTION( "cannot have multiple reporters with default output" ) {
                ...
                CHECK( !result );
                REQUIRE_THAT( result.errorMessage(),
                              ContainsSubstring( "Only one reporter may have "
    ...
    SECTION( "abort" ) {
        ...
        SECTION( "wait-for-keypress" ) {
            ...
            SECTION( "invalid options are reported" ) {
                ...
                REQUIRE_THAT( result.errorMessage(),
                              ContainsSubstring( "never" ) &&
                                  ContainsSubstring( "both" ) );
#endif
            }
        }
    }
    ...
    SECTION( "use-colour" ) {
        ...
        SECTION( "error" ) {
            ...
            CHECK_THAT( result.errorMessage(),
                        ContainsSubstring( "colour mode must be one of" ) );
        }
    }
...
TEST_CASE( "Parse rng seed in different formats",
    ...
    SECTION( "well formed cases" ) {
        ...
        std::tie( seed_string, seed_value ) =
            GENERATE( table<char const*, uint32_t>(
