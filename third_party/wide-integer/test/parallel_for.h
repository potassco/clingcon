...
void parallel_for(index_type start, index_type end, callable_function_type parallel_function) {
    ...
    static const unsigned int number_of_threads = ((number_of_threads_hint == 0U) ? 4U : number_of_threads_hint);

    // Set the size of a slice for the range functions.
    index_type n = index_type(end - start) + index_type(1);

    index_type slice = static_cast<index_type>(std::round(n / static_cast<double>(number_of_threads)));
...
template <typename index_type, typename callable_function_type>
void sequential_for(index_type start, index_type end, callable_function_type sequential_function) {
