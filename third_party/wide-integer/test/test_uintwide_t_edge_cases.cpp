...
using local_uint_type = boost::multiprecision::number<boost::multiprecision::uintwide_t_backend<local_digits2>,
                                                      boost::multiprecision::et_off>;

using boost_uint_backend_type =
    boost::multiprecision::cpp_int_backend<local_digits2, local_digits2, boost::multiprecision::unsigned_magnitude>;
...
bool math::wide_integer::test_uintwide_t_edge_cases() {
    ...
    const std::string str_three_quarter_effs_and_zeros =
        "0x" + std::string(std::string::size_type((local_digits2 / 4) * 3U), char('F')) +
    ...
    const std::string str_one_quarter_effs_and_zeros =
        "0x" + std::string(std::string::size_type((local_digits2 / 4) * 1U), char('F')) +
    ...
    const local_uint_type one_limb_effs_prior_to_half_and_zeros_local(
        local_uint_type(UINT32_C(0xFFFFFFFF)) << ((std::numeric_limits<local_uint_type>::digits / 2) - 32));
    const boost_uint_type one_limb_effs_prior_to_half_and_zeros_boost(
        boost_uint_type(UINT32_C(0xFFFFFFFF)) << ((std::numeric_limits<boost_uint_type>::digits / 2) - 32));
    ...
    const local_uint_type u_mid_local = u_three_quarter_effs_and_zeros_local /
                                        typename local_uint_type::backend_type::representation_type::limb_type(2U);
    const boost_uint_type u_mid_boost =
        u_three_quarter_effs_and_zeros_boost /
        typename std::iterator_traits<boost_uint_type::backend_type::limb_pointer>::value_type(2U);
    ...
    const bool result_is_ok = (result01_is_ok && result02_is_ok && result03_is_ok && result04_is_ok && result05_is_ok &&
                               result06_is_ok && result07_is_ok && result08_is_ok);
