...
TEST_CASE( "GENERATE capture macros", "[generators][internals][approvals]" ) {
    ...
    auto value2 = GENERATE_REF( Catch::Generators::GeneratorWrapper<int>(
        Catch::Detail::make_unique<TestGen>( nc ) ) );
...
TEST_CASE( "#2040 - infinite compilation recursion in GENERATE with MSVC",
           "[generators][compilation][approvals]" ) {
    int x = 42;
...
namespace {
    ...
    static bool is_multiple_of_3( int n ) { return n % 3 == 0; }
} // namespace

TEST_CASE( "GENERATE handles function (pointers)",
           "[generators][compilation][approvals]" ) {
...
TEST_CASE( "Filter generator throws exception for empty generator",
    ...
    REQUIRE_THROWS_AS( filter( []( int ) { return false; }, value( 3 ) ),
                       Catch::GeneratorException );
}

TEST_CASE( "from_range(container) supports ADL begin/end and arrays",
           "[generators][from-range][approvals]" ) {
    using namespace Catch::Generators;
