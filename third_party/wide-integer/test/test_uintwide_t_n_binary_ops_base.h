...
class test_uintwide_t_n_binary_ops_base : public test_uintwide_t_n_base {
  public:
    test_uintwide_t_n_binary_ops_base(const std::size_t count) : test_uintwide_t_n_base(count) {}
    ...
    virtual bool do_test(const std::size_t rounds) {
        ...
        for (std::size_t i = 0U; i < rounds; ++i) {
            ...
            std::cout << "test_binary_sqrt() boost compare with uintwide_t: round " << i
                      << ",  digits2: " << this->get_digits2() << std::endl;
  ...
  protected:
    using random_generator_type =
        std::mersenne_twister_engine<std::uint32_t, 32, 624, 397, 31, UINT32_C(0x9908B0DF), 11, UINT32_C(0xFFFFFFFF), 7,
                                     UINT32_C(0x9D2C5680), 15, UINT32_C(0xEFC60000), 18, UINT32_C(1812433253)>;
