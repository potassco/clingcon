...
namespace Catch {
    namespace Benchmark {
        namespace Detail {
            SampleAnalysis
            analyse( const IConfig& cfg, FDuration* first, FDuration* last ) {
...
namespace Catch {
    namespace Clara {
        ...
        Arg::parse( std::string const&, Detail::TokenStream tokens ) const {
            ...
            auto result =
                valueRef->setValue( static_cast<std::string>( token.token ) );
            if ( !result )
        ...
        Detail::InternalParseResult
        Opt::parse( std::string const&, Detail::TokenStream tokens ) const {
            auto validationResult = validate();
            if ( !validationResult )
                return Detail::InternalParseResult( validationResult );
        ...
        void Parser::writeToStream( std::ostream& os ) const {
            ...
            for ( auto& cols : rows ) {
                auto row = TextFlow::Column( CATCH_MOVE( cols.left ) )
                           ...
                           TextFlow::Spacer( 4 ) +
                           TextFlow::Column(
                               static_cast<std::string>( cols.descriptions ) )
        ...
        Parser::parse( std::string const& exeName,
            ...
            auto result = Detail::InternalParseResult::ok( Detail::ParseState(
                ParseResultType::NoMatch, CATCH_MOVE( tokens ) ) );
                ...
                for ( auto& parseInfo : parseInfos ) {
                    if ( parseInfo.parser->cardinality() == 0 ||
                        ...
                        result = parseInfo.parser->parse(
                            exeName,
                            CATCH_MOVE( result ).value().remainingTokens() );
                        if ( !result ) return result;
...
namespace Catch {
    ...
    JsonReporter::~JsonReporter() {
        ...
        assert( m_writers.size() == 1 &&
                "Only the top level object should be open" );
    ...
    void JsonReporter::testCaseEnded( TestCaseStats const& tcStats ) {
...

        {
            ...
            // We do not write the test case totals, because there will always
            // be just one test case here.
            // TODO: overall "result" -> success, skip, fail here? Or in partial
            // result?
        }
        // We do not write out stderr/stdout, because we instead wrote those out
        // in partial runs
    ...
    void
    JsonReporter::assertionStarting( AssertionInfo const& /*assertionInfo*/ ) {}
