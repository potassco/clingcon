...
TEST_CASE( "XmlEncode: UTF-8", "[XML][UTF-8][approvals]" ) {
    ...
    SECTION( "Invalid utf-8 strings" ) {
        SECTION( "Various broken strings" ) {
            CHECK( encode( "Here \xFF be \xF0\x9F\x91\xBE" ) ==
                   "Here \\xFF be 👾" );
            CHECK( encode( "\xFF" ) == "\\xFF" );
            CHECK( encode( "\xC5\xC5\xA0" ) == "\\xC5Š" );
            CHECK( encode( "\xF4\x90\x80\x80" ) ==
                   "\\xF4\\x90\\x80\\x80" ); // 0x110000 -- out of unicode range
        }
...
TEST_CASE( "XmlWriter writes boolean attributes as true/false",
           "[XML][XmlWriter]" ) {
...
TEST_CASE( "XmlWriter does not escape comments",
           "[XML][XmlWriter][approvals]" ) {
    using Catch::Matchers::ContainsSubstring;
    std::stringstream stream;
    {
        Catch::XmlWriter xml( stream );
...
TEST_CASE( "XmlWriter errors out when writing text without enclosing element",
           "[XmlWriter][approvals]" ) {
    std::stringstream stream;
    Catch::XmlWriter xml( stream );
...
TEST_CASE( "XmlWriter escapes text properly", "[XML][XmlWriter][approvals]" ) {
    ...
    {
        Catch::XmlWriter xml( stream );
        xml.scopedElement( "root" ).writeText(
            R"(Special chars need escaping: < > ' " &)" );
    }
...
TEST_CASE( "XmlWriter escapes attributes properly",
           "[XML][XmlWriter][approvals]" ) {
    using Catch::Matchers::ContainsSubstring;
    std::stringstream stream;
    {
        Catch::XmlWriter xml( stream );
        xml.scopedElement( "root" ).writeAttribute(
            "some-attribute", R"(Special chars need escaping: < > ' " &)" );
    }

    REQUIRE_THAT(
        stream.str(),
        ContainsSubstring(
            R"(some-attribute="Special chars need escaping: &lt; > ' &quot; &amp;")" ) );
}
