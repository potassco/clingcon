...
template <typename FloatingPointType, const std::int32_t LoExp10, const std::int32_t HiExp10>
FloatingPointType get_random_float() {
    using local_builtin_float_type = FloatingPointType;

    static std::uniform_real_distribution<local_builtin_float_type> dist_man(local_builtin_float_type(0.0F),
                                                                             local_builtin_float_type(1.0F));

    static std::uniform_int_distribution<unsigned> dist_sgn(0, 1);

    static std::uniform_int_distribution<std::int32_t> dist_e10(LoExp10, HiExp10);

    using std::pow;

    const std::int32_t p10 = dist_e10(engine_e10);
    const local_builtin_float_type e10 = pow(local_builtin_float_type(10.0F), local_builtin_float_type(p10));
...
template <const std::size_t MaxDigitsToGet, const std::size_t MinDigitsToGet = 2U>
void get_random_digit_string(std::string &str) {
    ...
    static std::uniform_int_distribution<unsigned> dist_len(MinDigitsToGet, MaxDigitsToGet);

    static std::uniform_int_distribution<unsigned> dist_first(1, 9);

    static std::uniform_int_distribution<unsigned> dist_following(0, 9);
...
bool math::wide_integer::test_uintwide_t_float_convert() {
    ...
    using boost_limb_type = typename std::iterator_traits<
        typename boost_uint_backend_type::cpp_int_backend::base_type::cpp_int_base::limb_pointer>::value_type;
