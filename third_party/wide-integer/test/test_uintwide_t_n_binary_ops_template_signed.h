...
template <const math::wide_integer::size_t MyDigits2, typename MyLimbType = std::uint32_t,
...
class test_uintwide_t_n_binary_ops_template_signed : public test_uintwide_t_n_binary_ops_base {
  private:
    ...
    using boost_uint_backend_type =
        boost::multiprecision::cpp_int_backend<digits2, digits2, boost::multiprecision::unsigned_magnitude>;
    using boost_sint_backend_type =
        boost::multiprecision::cpp_int_backend<digits2, digits2, boost::multiprecision::signed_magnitude>;
  ...
  public:
    test_uintwide_t_n_binary_ops_template_signed(const std::size_t count)
        : test_uintwide_t_n_binary_ops_base(count), a_local(), b_local(), a_local_signed(), b_local_signed(), a_boost(),
          b_boost(), a_boost_signed(), b_boost_signed() {}
    ...
    virtual void initialize() {
        ...
        get_equal_random_test_values_boost_and_local_n<local_uint_type, boost_uint_type, AllocatorType>(
            a_local.data(), a_boost.data(), size());
        get_equal_random_test_values_boost_and_local_n<local_uint_type, boost_uint_type, AllocatorType>(
            b_local.data(), b_boost.data(), size());
    ...
    virtual bool test_binary_add() const {
        ...
        my_concurrency::parallel_for(std::size_t(0U), size(), [&test_lock, &result_is_ok, this](std::size_t i) {
    ...
    virtual bool do_test(const std::size_t rounds) {
        ...
        for (std::size_t i = 0U; i < rounds; ++i) {
            std::cout << "initialize()       boost compare with uintwide_t: round " << i
                      << ",  digits2: " << this->get_digits2() << std::endl;
            this->initialize();

            std::cout << "test_binary_add()  boost compare with uintwide_t: round " << i
                      << ",  digits2: " << this->get_digits2() << std::endl;
            result_is_ok &= this->test_binary_add();

            std::cout << "test_binary_sub()  boost compare with uintwide_t: round " << i
                      << ",  digits2: " << this->get_digits2() << std::endl;
            result_is_ok &= this->test_binary_sub();

            std::cout << "test_binary_mul()  boost compare with uintwide_t: round " << i
                      << ",  digits2: " << this->get_digits2() << std::endl;
            result_is_ok &= this->test_binary_mul();

            std::cout << "test_binary_div()  boost compare with uintwide_t: round " << i
                      << ",  digits2: " << this->get_digits2() << std::endl;
            result_is_ok &= this->test_binary_div();

            std::cout << "test_binary_mod()  boost compare with uintwide_t: round " << i
                      << ",  digits2: " << this->get_digits2() << std::endl;
...
class test_uintwide_t_n_binary_ops_template_signed<math::wide_integer::size_t(64U), std::uint16_t, AllocatorType>
  ...
  public:
    test_uintwide_t_n_binary_ops_template_signed(const std::size_t count)
        : test_uintwide_t_n_binary_ops_base(count), a_local_signed(), b_local_signed(), a_native_signed(),
          b_native_signed() {}
    ...
    virtual bool test_binary_add() const {
        ...
        my_concurrency::parallel_for(std::size_t(0U), size(), [&test_lock, &result_is_ok, this](std::size_t i) {
            ...
            const native_sint_type c_native_signed = c_native_unsigned < UINT64_C(0x8000000000000000)
                                                         ? (native_sint_type)c_native_unsigned
                                                         : -(native_sint_type)(~c_native_unsigned + 1U);
    ...
    virtual bool test_binary_shr() const {
        ...
        my_concurrency::parallel_for(std::size_t(0U), size(), [&test_lock, &result_is_ok, this](std::size_t i) {
            ...
            const bool current_result_is_ok =
                (current_result_is_zero || (c_native_signed == (std::int64_t)c_local_signed));
    ...
    virtual bool do_test(const std::size_t rounds) {
        ...
        for (std::size_t i = 0U; i < rounds; ++i) {
            ...
            std::cout << "test_binary_mod1() boost compare with uintwide_t: round " << i
                      << ",  digits2: " << this->get_digits2() << std::endl;
            result_is_ok &= this->test_binary_mod1();

            std::cout << "test_binary_shr()  boost compare with uintwide_t: round " << i
                      << ",  digits2: " << this->get_digits2() << std::endl;
...
template <typename AllocatorType>
std::minstd_rand test_uintwide_t_n_binary_ops_template_signed<64U, std::uint16_t, AllocatorType>::my_eng;
template <typename AllocatorType>
std::uniform_int_distribution<>
    test_uintwide_t_n_binary_ops_template_signed<64U, std::uint16_t, AllocatorType>::my_distrib_0_to_63(UINT16_C(0),
                                                                                                        UINT16_C(63));
template <typename AllocatorType>
std::uniform_int_distribution<>
    test_uintwide_t_n_binary_ops_template_signed<64U, std::uint16_t, AllocatorType>::my_distrib_1_to_0xFFFF(
        UINT16_C(1), UINT16_C(0xFFFF));
