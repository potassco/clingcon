...
void get_random_big_uint(RandomEngineType &rng, UnsignedIntegralIteratorType it_out) {
    ...
    using distribution_type = math::wide_integer::uniform_int_distribution<std::numeric_limits<local_uint_type>::digits,
...
bool math::wide_integer::example009b_timed_mul_8_by_8() {
    ...
    for (typename std::vector<big_uint_type>::size_type i =
             static_cast<typename std::vector<big_uint_type>::size_type>(0U);
         i < local_a.size(); ++i) {
        get_random_big_uint(rng,
                            local_a.begin() + static_cast<typename std::vector<big_uint_type>::difference_type>(i));
        get_random_big_uint(rng,
                            local_b.begin() + static_cast<typename std::vector<big_uint_type>::difference_type>(i));
    }
    ...
    std::cout << "bits: " << std::numeric_limits<big_uint_type>::digits << ", kops_per_sec: " << std::fixed
              << std::setprecision(2) << kops_per_sec << ", count: " << count << std::endl;
