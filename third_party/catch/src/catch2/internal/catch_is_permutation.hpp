...
namespace Catch {
    namespace Detail {
        ...
        bool is_permutation( ForwardIter1 first_1,
                             ...
                             const Sentinel2 end_2,
                             Comparator cmp ) {
            // TODO: no optimization for stronger iterators, because we would
            // also have to constrain on sentinel vs not sentinel types
            ...
            // We need to handle case where at least one of the ranges has no
            // more elements
            if ( first_1 == end_1 || first_2 == end_2 ) {
                return first_1 == end_1 && first_2 == end_2;
            }

            // pair counting is n**2, so we pay linear walk to compare the sizes
            // first
