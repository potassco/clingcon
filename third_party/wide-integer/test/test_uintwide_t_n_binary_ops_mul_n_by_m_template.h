...
template <const math::wide_integer::size_t MyDigits2A, const math::wide_integer::size_t MyDigits2B,
...
class test_uintwide_t_n_binary_ops_mul_n_by_m_template : public test_uintwide_t_n_binary_ops_base {
  private:
    ...
    using boost_uint_backend_a_type =
        boost::multiprecision::cpp_int_backend<digits2a, digits2a, boost::multiprecision::unsigned_magnitude>;

    using boost_uint_backend_b_type =
        boost::multiprecision::cpp_int_backend<digits2b, digits2b, boost::multiprecision::unsigned_magnitude>;

    using boost_uint_backend_c_type = boost::multiprecision::cpp_int_backend<digits2a + digits2b, digits2a + digits2b,
  ...
  public:
    test_uintwide_t_n_binary_ops_mul_n_by_m_template(const std::size_t count)
        : test_uintwide_t_n_binary_ops_base(count), a_local(), b_local(), a_boost(), b_boost() {}
    ...
    virtual bool test_binary_mul() const {
        ...
        my_concurrency::parallel_for(std::size_t(0U), size(), [&test_lock, &result_is_ok, this](std::size_t i) {
            const boost_uint_c_type c_boost = boost_uint_c_type(a_boost[i]) * b_boost[i];

            const local_uint_c_type c_local =
                static_cast<local_uint_c_type>(a_local[i]) * static_cast<local_uint_c_type>(b_local[i]);
