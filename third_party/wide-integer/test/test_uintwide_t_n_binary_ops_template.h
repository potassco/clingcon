...
class test_uintwide_t_n_binary_ops_template : public test_uintwide_t_n_binary_ops_base {
  ...
  public:
    ...
    virtual bool test_binary_div() const {
        ...
        my_concurrency::parallel_for(std::size_t(0U), size(), [&result_is_ok, this, &dis, &test_lock](std::size_t i) {
            while (test_lock.test_and_set()) {
                ;
            }
            ...
            const boost_uint_type c_boost =
                a_boost[i] / (std::max)(boost_uint_type(1U), boost_uint_type(b_boost[i] >> right_shift_amount));
            const local_uint_type c_local =
                a_local[i] / (std::max)(local_uint_type(1U), (b_local[i] >> right_shift_amount));
