...
def adjust_version():
    ...
    post = 0
    for m in finditer(r'clingcon[_-]cffi-{}.post([0-9]+)'.format(escape(version)), pip):
        post = max(post, int(m.group(1)) + 1)

    for m in finditer(r'clingcon[_-]cffi-{}.post([0-9]+).*manylinux2014_'.format(escape(version)), pip):
