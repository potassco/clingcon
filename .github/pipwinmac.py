...
def adjust_version():
    ...
    '''
    with urlopen('https://test.pypi.org/simple/clingcon') as uh:
        pip = uh.read().decode()
    ...
    for m in finditer(r'clingcon-{}-{}-.*-{}'.format(escape(version), escape(python_tag()), escape(platform_tag())), pip):
        post = max(post, 1)

    for m in finditer(r'clingcon-{}\.post([0-9]+)-{}-.*-{}'.format(escape(version), escape(python_tag()), escape(platform_tag())), pip):
