...
template <const math::wide_integer::size_t MyWidth2, typename MyLimbType, typename EnableType = void>
...
class test_uintwide_t_n_binary_ops_mul_div_4_by_4_template<
    ...
    typename std::enable_if<(
        (std::numeric_limits<MyLimbType>::digits * 4 == MyWidth2) && (std::is_fundamental<MyLimbType>::value == true) &&
        (std::is_integral<MyLimbType>::value == true) && (std::is_unsigned<MyLimbType>::value == true))>::type>
  ...
  private:
    ...
    using native_uint_cntrl_type = typename math::wide_integer::detail::uint_type_helper<digits2>::exact_unsigned_type;
  ...
  public:
    test_uintwide_t_n_binary_ops_mul_div_4_by_4_template(const std::size_t count)
        : test_uintwide_t_n_binary_ops_base(count), a_local(), b_local(), a_cntrl(), b_cntrl() {}
    ...
    virtual bool do_test(const std::size_t rounds) {
        ...
        for (std::size_t i = 0U; i < rounds; ++i) {
            std::cout << "initialize()       native compare with uintwide_t: round " << i
                      << ",  digits2: " << this->get_digits2() << std::endl;
            this->initialize();

            std::cout << "test_binary_mul()  native compare with uintwide_t: round " << i
                      << ",  digits2: " << this->get_digits2() << std::endl;
            result_is_ok &= this->test_binary_mul();

            std::cout << "test_binary_div()  native compare with uintwide_t: round " << i
                      << ",  digits2: " << this->get_digits2() << std::endl;
    ...
    virtual bool test_binary_mul() const {
        ...
        my_concurrency::parallel_for(std::size_t(0U), size(), [&test_lock, &result_is_ok, this](std::size_t i) {
            const native_uint_cntrl_type c_cntrl = native_uint_cntrl_type(a_cntrl[i]) * b_cntrl[i];

            const local_uint_ab_type c_local =
                static_cast<local_uint_ab_type>(a_local[i]) * static_cast<local_uint_ab_type>(b_local[i]);
    ...
    virtual bool test_binary_div() const {
        ...
        test_uintwide_t_n_binary_ops_base::my_gen.seed(
            static_cast<typename random_generator_type::result_type>(std::clock()));
        std::uniform_int_distribution<> dis(1, static_cast<int>(digits2 - 1U));

        bool result_is_ok = true;

        my_concurrency::parallel_for(std::size_t(0U), size(), [&test_lock, &result_is_ok, this, &dis](std::size_t i) {
            while (test_lock.test_and_set()) {
                ;
            }
            ...
            const native_uint_cntrl_type c_cntrl =
                a_cntrl[i] /
                (std::max)(native_uint_cntrl_type(1U), native_uint_cntrl_type(b_cntrl[i] >> right_shift_amount));
            const local_uint_ab_type c_local =
                a_local[i] / (std::max)(local_uint_ab_type(1U), (b_local[i] >> right_shift_amount));
  ...
  private:
    ...
    template <typename OtherLocalUintType, typename OtherCntrlUintType>
    static void get_equal_random_test_values_cntrl_and_local_n(OtherLocalUintType *u_local, OtherCntrlUintType *u_cntrl,
        ...
        using distribution_type =
            math::wide_integer::uniform_int_distribution<other_local_uint_type::my_width2,
                                                         typename other_local_uint_type::limb_type>;
