...
void VariableCreator::setLELit(const Restrictor::ViewIterator& it, Literal l)
{
    if (it.view().reversed())
...
void VariableCreator::setGELit(const Restrictor::ViewIterator& it, Literal l)
{
    if (it.view().reversed())
    {
        setLELit(ViewIterator::viewToVarIterator(it),l);
        return;
    }
    if (getDomainSize(it.view()) == it.numElement()) /// end iterator, after range
    {
        s_.setEqual(l,s_.falseLit());
        return;
    }
    Variable v = it.view().v;
    isValid(v);
...
bool VariableCreator::createOrderLiterals(const Variable& i)
{
    ...
    if (size && ( 0 < conf_.minLitsPerVar || conf_.minLitsPerVar==-1) && (conf_.minLitsPerVar >= 0 ? orderLitMemory_[i].numLits()<conf_.minLitsPerVar : orderLitMemory_[i].numLits()<size+1))
    {
        unsigned int add = std::max((conf_.minLitsPerVar==-1 ? size: (int64)(conf_.minLitsPerVar)),(int64)0);
        if (add>0)
        {
    ...
    if (orderLitMemory_[i].hasNoLiteral(size))
        ...
        if (!s_.createClause(LitVec{orderLitMemory_[i].getLiteral(size)}))
            return false;
    return true;
}
...
bool VariableCreator::restrictDomainsAccordingToLiterals()
{
    ...
    for (auto i = equalLits_.begin(); i != equalLits_.end();)
    {
        ...
        if (s_.isFalse(i->second))
        {
            if (!removeFromView(View(i->first.first),i->first.second))
                return false;
            equalLits_.erase(i++);
        }
        else
...
bool VariableCreator::domainChange(const Variable& var, const Domain& d)
{
    ...
    if (hasOrderLitMemory(var))
    {
        ...
        if (orderLitMemory_[var].hasMap())
        {
            ...
            auto k = keep.begin();
            bool inside = true; /// true if (k and k+1) means keep, false if it means no keep
            size_t move = start;
            for (auto h = m.begin(); h != m.end();)
            {
                /// everyting before start to false
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (h->first >= (*(k))-r.begin() && h->first < (*(k+1))-r.begin())
                        {
                            ...
                            {
                                /// remove from map, but make it equal its least partner
        ...
        if (orderLitMemory_[var].hasVector())
        {
            ...
            for (auto i = keep.begin(); i != keep.end(); ++i)
            {
                ...
                insert+=offset;
                /// everything that gets deleted needs to be relocated to the first valid value
                if (i+2!=keep.end())
                {
                    ...
                    {
                        if (!orderLitMemory_[var].getVector()[elems].flagged())
                        {
                            if (orderLitMemory_[var].getVector()[oldindex].flagged())
                                orderLitMemory_[var].getVector()[oldindex] = orderLitMemory_[var].getVector()[elems];
                            else if (!orderLitMemory_[var].hasMap()) // if not already done
                                s_.setEqual(orderLitMemory_[var].getLiteral(elems), orderLitMemory_[var].getLiteral(oldindex));
                        }
                    }
